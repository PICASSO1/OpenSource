...
typedef int(*mapping_add)(struct if_mapping *ifnode, int *active, char *pos, size_t len, struct add_extra *extra, int linenum);

// Prototype for comparing the selector of two mapping. Return 0 if matches.
typedef int(*mapping_cmp)(struct if_mapping *ifnode, struct if_mapping *target);

// Prototype for extracting selector value from live interface
typedef int(*mapping_get)(int skfd, const char *ifname, struct if_mapping *target, int flag);
...
static int mapping_addmac(struct if_mapping *ifnode, int *active, char *pos, size_t len, struct add_extra *extra, int linenum);

static int mapping_cmpmac(struct if_mapping *ifnode, struct if_mapping *target);

static int mapping_getmac(int skfd, const char *ifname, struct if_mapping *target, int flag);
...
static int mapping_getarp(int skfd, const char *ifname, struct if_mapping *target, int flag);
...
static int mapping_addbusinfo(struct if_mapping *ifnode, int *active, char *pos, size_t len, struct add_extra *extra, int linenum);
...
static int mapping_getdriverbusinfo(int skfd, const char *ifname, struct if_mapping *target, int flag);
...
static int mapping_addirq(struct if_mapping *ifnode, int *active, char *pos, size_t len, struct add_extra *extra, int linenum);

static int mapping_cmpirq(struct if_mapping *ifnode, struct if_mapping *target);

static int mapping_getbaseaddrirq(int skfd, const char *ifname, struct if_mapping *target, int flag);
...
static int mapping_getpcmciaslot(int skfd, const char *ifname, struct if_mapping *target, int flag);

static int mapping_addsysfs(struct if_mapping *ifnode, int *active, char *pos, size_t len, struct add_extra *extra, int linenum);

static int mapping_cmpsysfs(struct if_mapping *ifnode, struct if_mapping *target);

static int mapping_getsysfs(int skfd, const char *ifname, struct if_mapping *target, int flag);
...
struct if_mapping *mapping_list = (struct if_mapping *)NULL;
...
const struct mapping_selector selector_list[] = {
	...
	{"driver", &mapping_adddriver, &mapping_cmpdriver, &mapping_getdriverbusinfo},
	{"businfo", &mapping_addbusinfo, &mapping_cmpbusinfo, &mapping_getdriverbusinfo},
	{"firmware", &mapping_addfirmware, &mapping_cmpfirmware, &mapping_getdriverbusinfo},
	// Base Address and IRQ from ifconfig
	{"baseaddress", &mapping_addbaseaddr, &mapping_cmpbaseaddr, &mapping_getbaseaddrirq},
...
{
	const char *p = (char *)NULL, *v = (char *)NULL;
	int n = -1, ret = -1;
...
// Ask the kernel to change the name of an interface. That's what we want to do. All the rest is to make sure we call this appropriately. 
static int if_set_name(int skfd, const char *oldname, const char *newname, char *retname)
{
	struct ifreq ifr;
	char *star = (char *)NULL;
	int ret = -1;
...
static int mapping_addmac(struct if_mapping *ifnode, int *active, char *string, size_t len, struct add_extra *extra, int linenum)
{
	size_t n = 0;
	...
	if (strchr(ifnode->mac_filter, '*') != NULL) {
		...
		ifnode->mac_len = iw_mac_aton(ifnode->mac_filter, ifnode->mac, MAX_MAC_LEN);
		if (ifnode->mac_len == 0) {
		...
		if ((ifnode->mac_len == 6) && (!memcmp(&ifnode->mac, &zero_mac, 6)))
			fprintf(stderr, "Warning: MAC address is null at line %d, this is dangerous...\n", linenum);
	...
	if (verbose) {
		fprintf(stderr, "Parsing : Added %s MAC address `%s' from line %d.\n", \
			ifnode->active[SELECT_MAC] == HAS_MAC_FILTER? "filter": "exact", ifnode->mac_filter, linenum);
	}
...
static int mapping_cmpmac(struct if_mapping *ifnode, struct if_mapping *target)
{
	...
	if (ifnode->active[SELECT_MAC] == HAS_MAC_FILTER) {
		...
		return ((ifnode->mac_len != target->mac_len) || memcmp(ifnode->mac, target->mac, ifnode->mac_len));
	}
}
...
static int mapping_getmac(int skfd, const char *ifname, struct if_mapping *target, int flag)
{
	...
	if ((flag == HAS_MAC_FILTER) || verbose) {
		// Convert to ASCII
		iw_mac_ntop(target->mac, target->mac_len, target->mac_filter, sizeof(target->mac_filter));
    }
	...
	if (verbose)
		fprintf(stderr, "Querying %s : Got MAC address `%s' and ARP/Link Type `%d'.\n", ifname, target->mac_filter, target->hw_type);

	return 0;
}

// Add a ARP/Link type selector to a mapping
static int mapping_addarp(struct if_mapping *ifnode, int *active, char *string, size_t len, struct add_extra *extra, int linenum)
{
	size_t n = 0;
...
static int mapping_adddriver(struct if_mapping *ifnode, int *active, char *string, size_t len, struct add_extra *extra, int linenum)
{
	// Avoid "Unused parameter" warning
	extra = extra;
	...
	if (verbose)
		fprintf(stderr, "Parsing : Added Driver name `%s' from line %d.\n", ifnode->driver, linenum);
...
static int mapping_addbusinfo(struct if_mapping *ifnode, int *active, char *string, size_t len, struct add_extra *extra, int linenum)
{
#if 0
	size_t n = 0;
#endif
	...
	if (verbose)
		fprintf(stderr, "Parsing : Added Bus Info `%s' from line %d.\n", ifnode->bus_info, linenum);
...
static int mapping_addfirmware(struct if_mapping *ifnode, int *active, char *string, size_t len, struct add_extra *extra, int linenum)
{
	// Avoid "Unused parameter" warning
	extra = extra;
	...
	if (verbose)
		fprintf(stderr, "Parsing : Added Firmware Revision `%s' from line %d.\n", ifnode->fw_version, linenum);
...
static int mapping_getdriverbusinfo(int skfd, const char *ifname, struct if_mapping *target, int flag)
{
	...
	if (target->active[SELECT_DRIVER] || target->active[SELECT_BUSINFO] || target->active[SELECT_FIRMWARE])
		return 0;
	...
	if (ret < 0) {
		...
		if (verbose)
			fprintf(stderr, "Error: Can't read driver/bus-info on interface `%s' : %s\n", ifname, strerror(errno));
	...
	if (verbose)
		fprintf(stderr, "Querying %s : Got Driver name `%s', Bus Info `%s' and Firmware `%s'.\n", ifname, target->driver, target->bus_info, target->fw_version);

	return 0;
}

// Add a Base Address selector to a mapping
static int mapping_addbaseaddr(struct if_mapping *ifnode, int *active, char *string, size_t len, struct add_extra *extra, int linenum)
{
	size_t n = 0;
	...
	if (verbose)
		fprintf(stderr, "Parsing : Added Base Address `0x%X' from line %d.\n", ifnode->base_addr, linenum);
...
static int mapping_addirq(struct if_mapping *ifnode, int *active, char *string, size_t len, struct add_extra *extra, int linenum)
{
	size_t n = 0;
	unsigned int irq = 0U;
	...
	if (verbose)
		fprintf(stderr, "Parsing : Added IRQ `%d' from line %d.\n", ifnode->irq, linenum);
...
static int mapping_getbaseaddrirq(int skfd, const char *ifname, struct if_mapping *target, int flag)
{
	...
	if (ret < 0) {
		...
		if (verbose)
			fprintf(stderr, "Error: Can't read base address/irq on interface `%s' : %s\n", ifname, strerror(errno));
	...
	if (ifr.ifr_map.base_addr >= 0x100)
		target->base_addr = ifr.ifr_map.base_addr; target->active[SELECT_BASEADDR] = 1;
    
	target->irq = ifr.ifr_map.irq;
	target->active[SELECT_IRQ] = 1;

	if (verbose)
		fprintf(stderr, "Querying %s : Got Base Address `0x%X' and IRQ `%d'.\n", ifname, target->base_addr, target->irq);

	return 0;
}

// Add a Wireless Protocol selector to a mapping
static int mapping_addiwproto(struct if_mapping *ifnode, int *active, char *string, size_t len, struct add_extra *extra, int linenum)
{
	// Avoid "Unused parameter" warning
	extra = extra;
	...
	if (verbose)
		fprintf(stderr, "Parsing : Added Wireless Protocol `%s' from line %d.\n", ifnode->iwproto, linenum);
...
static int mapping_getiwproto(int skfd, const char *ifname, struct if_mapping *target, int flag)
{
	...
	if (verbose)
		fprintf(stderr, "Querying %s : Got Wireless Protocol `%s'.\n", ifname, target->iwproto);

	return 0;
}

// Add a Pcmcia Slot selector to a mapping
static int mapping_addpcmciaslot(struct if_mapping *ifnode, int *active, char *string, size_t len, struct add_extra *extra, int linenum)
{
	size_t n = 0;
...
static int mapping_getpcmciaslot(int skfd, const char *ifname, struct if_mapping *target, int flag)
{
	...
	// Read each line of file getline is a GNU extension :-( The buffer is recycled and increased as needed by getline. 
	while (getline(&linebuf, &linelen, stream) > 0) {
		char *p = (char *)NULL;
		size_t n = 0, k = 0;
		int pcmcia_slot = -1, i = -1;
		...
		if (!strcmp(p, ifname)) {
			...
			if (verbose)
				fprintf(stderr, "Querying %s : Got Pcmcia Slot `%d'.\n", ifname, target->pcmcia_slot);
...
static int mapping_addsysfs(struct if_mapping *ifnode, int *active, char *string, size_t len, struct add_extra *extra, int linenum)
{
	int findex = -1;		// filename index
	char *sdup = (char *)NULL;
	...
	if (verbose)
		fprintf(stderr, "Parsing : Added SYSFS filename `%s' value `%s' from line %d.\n", sysfs_global.filename[findex], ifnode->sysfs[findex], linenum);
...
static int mapping_cmpsysfs(struct if_mapping *ifnode, struct if_mapping *target)
{
	...
	for (findex = 0; findex < sysfs_global.filenum; findex++) {
		...
		if (ifnode->sysfs[findex] != NULL) {
			...
			if ((target->sysfs[findex] == NULL) || (fnmatch(ifnode->sysfs[findex], target->sysfs[findex], FNM_CASEFOLD)))
...
{
	FILE *stream = (FILE *)NULL;
	char *fname = (char *)NULL, *sdup = (char *)NULL, *linebuf = (char *)NULL;
	size_t linelen = 0; 
	int fnsize = -1, findex = -1;		// filename index
	...
	if (target->sysfs_devpath == NULL) {
		...
		if (sysfs_global.root == NULL) {
			...
			while (getline(&linebuf, &linelen, stream) > 0) {
				int i = -1;
				size_t n = 0, toklen[3] = {0, 0, 0};
				char *token[3], *p = (char *)NULL;
		...
		// Construct devpath for this interface. Reserve enough space to replace name without realloc.
		...
		target->sysfs_devplen = sprintf(fname, "%s/class/net/%s", sysfs_global.root, ifname);
	...
	for (findex = 0; findex < sysfs_global.filenum; findex++) {
		...
		fnsize = (target->sysfs_devplen + 1 + strlen(sysfs_global.filename[findex]) + 1);
		fname = malloc(fnsize);
		if (fname == NULL) {
		...
		sprintf(fname, "%s/%s", target->sysfs_devpath, sysfs_global.filename[findex]);
		...
		if (n <= 0) {
			...
			*/
			...
			// I must note that the API for readlink() is very bad, which force us to have this ugly code. Yuck !
			int allocsize = 128;		// 256 = Good start
			int retry = 16, pathlen= -1;
			char *linkpath = (char *)NULL;
			...
			if (pathlen > 0)		// We have a symlink ;-) Terminate the string.
				linkpath[pathlen] = '\0';
			else {
				// Error !
				free((void *)linkpath);
			...
			//	if (!strcmp(fname + strlen(fname) - 3, "/.."))
				{
					...
					// Some sysfs attribute are void for some interface, we may have a real directory, or we may have permission issues... 
					if (verbose)
...
static int mapping_addprevname(struct if_mapping *ifnode, int *active, char *string, size_t len, struct add_extra *extra, int linenum)
{
	// Avoid "Unused parameter" warning
	extra = extra;
	...
	if (verbose)
		fprintf(stderr, "Parsing : Added Old Interface Name `%s' from line %d.\n", ifnode->prevname, linenum);
...
{
	struct if_mapping *ifnode = (struct if_mapping *)NULL;
	char *star = (char *)NULL;
	...
	* names and know what they are doing, so don't bother them... */
	if ((!force_takeover) && ((!strcmp(ifnode->ifname, "eth0")) || (!strcmp(ifnode->ifname, "wlan0"))))
		fprintf(stderr, "Warning: Interface name is `%s' at line %d, can't be mapped reliably.\n", ifnode->ifname, linenum);
...
static inline const struct mapping_selector *selector_find(const char *string, size_t slen, int linenum)
{
	const struct mapping_selector *found = (struct mapping_selector *)NULL;
	int ambig = 0, i = -1;
	...
	if (found == NULL) {
		...
		return (struct mapping_selector *)NULL;
	}
...
// Read the configuration file and extract all valid mappings and their selectors.
static int mapping_readfile(const char *filename)
{
	FILE *stream = (FILE *)NULL;
	char *linebuf = (char *)NULL;
	size_t linelen = 0; 
	...
	// Check filename
	if (!strcmp(filename, "-"))		// Read from stdin
		stream = stdin;
	else {
	...
	while (getline(&linebuf, &linelen, stream) > 0) {
		...
		while (*p != '\0') {
			const struct mapping_selector *selector = (struct mapping_selector *)NULL;
			struct add_extra *extra = (struct add_extra *)NULL;
			...
			if (*p == '"') {
				...
				e = strchr(p, '"');
				if (e == NULL) {
					fprintf(stderr, "Error: unterminated quoted value on line %d\n", linenum);
					ret = -1;
					break;				// Line ended
				}
				n = e - p;
		...
		// We add a mapping only if it has at least one selector and if all selectors were parsed properly. 
		if (ret < 0) {		// If we have not yet printed an error, now is a good time ;-)
			if (ret == -13)
...
{
	struct if_mapping *target = (struct if_mapping *)NULL;
	int i = -1;
	// Create mapping, zero it
	...
	for (ifnode = mapping_list; ifnode != NULL; ifnode = ifnode->next) {
		...
		for (i = 0; i < SELECT_NUM; i++) {
			...
			if (ifnode->active[i] != 0) {
				...
				if ((target->active[i] == 0) || (selector_list[i].cmp_fn(ifnode, target) != 0)) {
					matches = 0;
					break;
				}
			}
...
// Probe interfaces based on our list of mappings. This is the default, but usually not the best way to do it.
static void probe_mappings(int skfd)
{
	struct if_mapping *ifnode = (struct if_mapping *)NULL;
	struct ifreq ifr;
 ...
 */
static void probe_debian(int skfd)
{
	FILE *stream = (FILE *)NULL;
	char *linebuf = (char *)NULL;
	size_t linelen = 0; 
...
static int process_rename(int skfd, char *ifname, char *newname)
{
	...
	// Always print out the *new* interface name so that the calling script can pick it up and know where its interface has gone. 
	printf("%s\n", retname);
	// Done
	return 0;
}
...
{
	struct if_mapping *target = (if_mapping *)NULL;
	const struct if_mapping *mapping = (struct if_mapping *)NULL;
	char retname[IFNAMSIZ + 1];
	...
	// If user specified a new name, keep only interfaces that would match the new name... 
	...
	* That would be tricky to do... */
	...
	if (print_newname) {
		if (!udev_output) {
			...
			/* udev likes to call us as an IMPORT action. This means that we need to return udev the environment variables changed.
...
static inline int process_iflist(int skfd, char *args[], int count)
{
	num_takeover = 0;
	// Just do it
	...
	* Jean II */
	if (force_takeover && num_takeover)					// Play it again, Sam...
		iw_enum_devices(skfd, &process_ifname, args, count);
...
int main(int argc, char *argv[])
{
	...
	int use_probe = 0, is_debian = 0, skfd = -1, ret = -1;
	...
	if (ifname != NULL) {
		...
		// Rename all system interfaces. Mostly used for boot time processing (from init scripts).
