...
static void display_info(struct wireless_info *info, char *ifname)
{
	...
	if (info->has_retry) {
		...
		if (info->retry.disabled)
			...
			if (info->retry.flags & IW_RETRY_TYPE) {
				iw_print_retry_value(buffer, sizeof(buffer), info->retry.value, info->retry.flags, info->range.we_version_compiled);
				printf("%s", buffer);
			}
	...
	// Note : we display only one parameter, period or timeout. If a device (such as HiperLan) has both, the user need to use iwlist... 
	...
	if (info->has_stats) {
		...
		if (info->range.we_version_compiled > 11) {
			printf("          Rx invalid nwid:%d  Rx invalid crypt:%d  Rx invalid frag:%d\n          Tx excessive retries:%d  Invalid misc:%d   Missed beacon:%d\n", \
				info->stats.discard.nwid, info->stats.discard.code, info->stats.discard.fragment, info->stats.discard.retries, info->stats.discard.misc, info->stats.miss.beacon);
		}
		else
...
// Set ESSID
static int set_essid_info(skfd, ifname, args, count)
int skfd;
char *ifname;
char *args[];		// Command line args
int count;				// Args count
{
	...
	if ((!strcasecmp(args[0], "off")) || (!strcasecmp(args[0], "any"))) {
		...
		if (!strcasecmp(args[0], "on")) {
			...
			i = 0;
			// '-' or '--' allow to escape the ESSID string, allowing to set it to the string "any" or "off". This is a big ugly, but it will do for now
...
// Set Mode
static int set_mode_info(skfd, ifname, args, count)
int skfd;
char *ifname;
char *args[];		// Command line args
int count;				// Args count
...
static int set_freq_info(skfd, ifname, args, count)
int skfd;
char *ifname;
char *args[];		// Command line args
int count;				// Args count
...
// Set Bit Rate
static int 
set_bitrate_info(skfd, ifname, args, count)
int skfd;
char *ifname;
char *args[];		// Command line args
int count;				// Args count
...
// Set encryption
static int 
set_enc_info(skfd, ifname, args, count)
int skfd;
char *ifname;
char *args[];		// Command line args
int count;				// Args count
{
	...
	if (!strcasecmp(args[0], "on")) {
		...
		int gotone = 0, oldone = -1, keylen = -1, temp = -1;
...
static int 
set_power_info(skfd, ifname, args, count)
int skfd;
char *ifname;
char *args[];		// Command line args
int count;				// Args count
...
// Set Nickname
static int set_nick_info(skfd, ifname, args, count)
int skfd;
char *ifname;
char *args[];		// Command line args
int count;				// Args count
...
// Set commit
static int
set_nwid_info(skfd, ifname, args, count)
int skfd;
char *ifname;
char *args[];		// Command line args
int count;				// Args count
...
// Set AP Address
static int 
set_apaddr_info(skfd, ifname, args, count)
int skfd;
char *ifname;
char *args[];		// Command line args
int count;				// Args count
...
// Set Tx Power
static int 
set_txpower_info(skfd, ifname, args, count)
int skfd;
char *ifname;
char *args[];		// Command line args
int count;				// Args count
		...
		if (!strcasecmp(args[0], "auto"))
			...
			if (!strcasecmp(args[0], "on")) {
				...
				if (!strcasecmp(args[0], "fixed")) {
					...
					// Check if milliWatt
					// We authorise a single 'm' as a shorthand for 'mW', on the other hand a 'd' probably means 'dBm'... 
...
// Set Sensitivity
static int 
set_sens_info(skfd, ifname, args, count)
int skfd;
char *ifname;
char *args[];		// Command line args
int count;				// Args count
...
// Set Retry Limit
static int 
set_retry_info(skfd, ifname, args, count)
int skfd;
char *ifname;
char *args[];		// Command line args
int count;				// Args count
...
// Set RTS Threshold
static int
set_rts_info(skfd, ifname, args, count)
int skfd;
char *ifname;
char *args[];		// Command line args
int count;				// Args count
...
static int
set_frag_info(skfd, ifname, args, count)
int skfd;
char *ifname;
char *args[];		// Command line args
int count;			// Args count
...
// Set Modulation
static int
set_modulation_info(skfd, ifname, args, count)
int skfd;
char *ifname;
char *args[];		// Command line args
int count;			// Args count
