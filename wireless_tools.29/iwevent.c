...
struct wireless_iface *interface_cache = (struct wireless_iface *)NULL;
...
{
	struct wireless_iface *curr = (struct wireless_iface *)NULL;
...
{
	struct wireless_iface *curr = (struct wireless_iface *)NULL;
	struct wireless_iface *prev = (struct wireless_iface *)NULL;
	struct wireless_iface *next = (struct wireless_iface *)NULL;
...
static inline int 
print_event_token(event, iw_range, has_range)
...
int has_range;
{
	...
	switch (event->cmd) {
		...
		case SIOCGIWESSID: {
				...
				if (event->u.essid.flags) {		// Does it have an ESSID index ?
		...
		case IWEVMICHAELMICFAILURE:
			if (event->u.data.length >= sizeof(struct iw_michaelmicfailure)) {
				...
				printf("Michael MIC failure flags:0x%X src_addr:%s tsc:%s\n", mf.flags, \
					iw_saether_ntop(&mf.src_addr, buffer2), iw_hexdump(buffer, sizeof(buffer),
...
static inline int print_event_stream(int ifindex, char *data, int len)
{
	...
	struct timezone tz;
	struct wireless_iface *wireless_data = (struct wireless_iface *)NULL;
...
// Dump the events we receive from rtnetlink. This code is mostly from Casey

// Respond to a single RTM_NEWLINK event from the rtnetlink socket.
static int LinkCatcher(struct nlmsghdr *nlh)
{
	struct ifinfomsg *ifi = (struct ifinfomsg *)NULL;
#if 0
	...
	if (nlh->nlmsg_len > NLMSG_ALIGN(sizeof(struct ifinfomsg))) {
		...
		while (RTA_OK(attr, attrlen)) {
			...
			if (attr->rta_type == IFLA_WIRELESS) {
				// Go to display it
				print_event_stream(ifi->ifi_index, (char *)attr + \
					RTA_ALIGN(sizeof(struct rtattr)), attr->rta_len - RTA_ALIGN(sizeof(struct rtattr)));
			}
...
static inline void handle_netlink_events(struct rtnl_handle *rth)
{
	while (1) {
		...
		struct nlmsghdr *h = (struct nlmsghdr *)NULL;
		int amt = -1;
		char buf[8192];
