...
static int print_nwid(int skfd, const char *ifname, int format)
{
	struct iwreq wrq;
...
static int print_ap(int skfd, const char *ifname, int format)
{
	struct iwreq wrq;
	char buffer[64];
...
static int print_freq(int skfd, const char *ifname, int format)
{
	struct iwreq wrq;
	double freq;
	char buffer[64];
...
static int print_channel(int skfd, const char *ifname, int format)
{
	struct iwreq wrq;
...
static int print_mode(int skfd, const char *ifname, int format)
{
	struct iwreq wrq;
...
static int print_one_device(int skfd, int format, int wtype, const char *ifname)
{
	int ret = -1;
...
int main(int	argc, char **argv)
{
	...
	while ((opt = getopt_long(argc, argv, "acfhmprs", long_opts, NULL)) > 0) {
		switch (opt) {
			case 'a':		// User wants AP/Cell Address
				wtype = WTYPE_AP;
				break;
			case 'c':		// User wants channel only
				wtype = WTYPE_CHANNEL;
				break;
			case 'f':		// User wants frequency/channel
				wtype = WTYPE_FREQ;
				break;
			case 'm':		// User wants the mode
				wtype = WTYPE_MODE;
				break;
			case 'p':		// User wants the protocol
				wtype = WTYPE_PROTO;
				break;
			case 'h':
				iw_usage(0);
				break;
			case 'r':		// User wants a Raw format
				format = FORMAT_RAW;
				break;
			case 's':		// User wants a Scheme format
