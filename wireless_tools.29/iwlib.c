...
void iw_enum_devices(int skfd, iw_enum_handler fn, char *args[], int count)
{
	...
	FILE *fh = (FILE *)NULL;
	struct ifconf ifc;
	struct ifreq *ifr = (struct ifreq *)NULL;
	int i = -1;
#ifndef IW_RESTRIC_ENUM
	...
	if (fh != NULL) {
		...
		while (fgets(buff, sizeof(buff), fh)) {
			...
			* cases fgets return a line with only a newline. */
			...
			else	// Got it, print info about this interface
...
static int 
print_iface_version_info(skfd, ifname, args, count)
int skfd;
char *ifname;
...
{
	int we_kernel_version = -1, skfd = -1;		// generic raw socket desc.
...
int iw_get_range_info(int skfd, const char *ifname, iwrange *range)
{
	struct iwreq wrq;
	char buffer[sizeof(iwrange) * 2];	// Large enough
	union iw_range_raw *range_raw = (union iw_range_raw *)NULL;
	// Cleanup
...
int iw_get_priv_info(int skfd, const char *ifname, iwprivargs **ppriv)
{
	...
	* sizes. Jean II */
	do {
		...
		// Failed. We probably need a bigger buffer. Check if the kernel gave us any hints.
...
int iw_protocol_compare(const char *protocol1, const char *protocol2)
{
	...
	if ((!strncmp(protocol1, dot11, strlen(dot11))) && (!strncmp(protocol2, dot11, strlen(dot11)))) {
		const char *sub1 = protocol1 + strlen(dot11);
		const char *sub2 = protocol2 + strlen(dot11);
		unsigned int	i = 0;
		int isds1 = 0, isds2 = 0, is5g1 = 0, is5g2 = 0;
	...
	if (has_range && ((qual->level != 0) || (qual->updated & (IW_QUAL_DBM | IW_QUAL_RCPI)))) {
		...
		if (qual->updated & IW_QUAL_RCPI) {
			...
			if (!(qual->updated & IW_QUAL_LEVEL_INVALID)) {
				...
				len = snprintf(buffer, buflen, "Signal level%c%g dBm  ", qual->updated & IW_QUAL_LEVEL_UPDATED? '=' : ':', rcpilevel);
				buffer += len;
				buflen -= len;
			}
			...
			if (!(qual->updated & IW_QUAL_NOISE_INVALID)) {
				...
				len = snprintf(buffer, buflen, "Noise level%c%g dBm", qual->updated & IW_QUAL_NOISE_UPDATED? '=' : ':', rcpinoise);
			}
		}
		else {
			...
			if ((qual->updated & IW_QUAL_DBM) || (qual->level > range->max_qual.level)) {
				...
				if (!(qual->updated & IW_QUAL_NOISE_INVALID)) {
					...
					len = snprintf(buffer, buflen, "Noise level%c%d dBm", qual->updated & IW_QUAL_NOISE_UPDATED? '=': ':', dbnoise);
				}
			}
			else {
				...
				if (!(qual->updated & IW_QUAL_NOISE_INVALID)) {
					len = snprintf(buffer, buflen, "Noise level%c%d/%d", \
						qual->updated & IW_QUAL_NOISE_UPDATED? '=': ':', qual->noise, range->max_qual.noise);
				}
			}
		}
	}
...
void iw_print_key(buffer, buflen, key, key_size, key_flags)
char *buffer;
int buflen;
 ...
 */
void iw_print_timeval(char *buffer, int buflen, const struct timeval *timev, const struct timezone *tz)
{
	int s = (timev->tv_sec - tz->tz_minuteswest * 60) % 86400;;

	snprintf(buffer, buflen, "%02d:%02d:%02d.%06u", s / 3600, (s % 3600) / 60, s % 60, (u_int32_t) timev->tv_usec);
...
int iw_check_mac_addr_type(int skfd, const char *ifname)
{
...
#ifdef DEBUG
	{
		...
		printf("Hardware : %d - %s\n", ifr.ifr_hwaddr.sa_family, iw_saether_ntop(&ifr.ifr_hwaddr, buf));
	}
#endif
	return 0;
}
...
int iw_check_if_addr_type(int skfd, const char *ifname)
{
	...
	strncpy(ifr.ifr_name, ifname, IFNAMSIZ);
	if ((ioctl(skfd, SIOCGIFADDR, &ifr) < 0) || (ifr.ifr_addr.sa_family !=  AF_INET)) {
		// Deep trouble...
...
int iw_check_addr_type(int skfd, char *ifname)
{
...
// Ask the kernel for the MAC address of an interface.
 int iw_get_mac_addr(int skfd, const char *ifname, struct ether_addr *eth, unsigned short *ptype)
{
	struct ifreq ifr;
	int ret = -1;
	// Prepare request
...
{
	sprintf(buf, "%02X:%02X:%02X:%02X:%02X:%02X", eth->ether_addr_octet[0], eth->ether_addr_octet[1], \
		eth->ether_addr_octet[2], eth->ether_addr_octet[3], eth->ether_addr_octet[4], eth->ether_addr_octet[5]);
...
{
	int maclen = iw_mac_aton(orig, (unsigned char *) eth, ETH_ALEN);
...
int iw_in_inet(char *name, struct sockaddr *sap)
{
	...
	struct netent *np = (struct netent *)NULL;
 ...
 */
void iw_init_event_stream(stream, data, len)
...
#endif
	// Extract the event header (to get the event id). Note : the event may be unaligned, therefore copy... 
	...
	if (event_type == IW_HEADER_TYPE_POINT) {
		...
		if (extra_len > 0) {
			...
			if (descr == NULL)	// Can't check payload -> unsafe...
				...
				// Those checks are actually pretty hard to trigger, because of the checks done in the kernel... 
				...
				* Fixing that in the kernel would break 64 bits userspace. */
				...
				// Discard bogus events which advertise more tokens than what they carry... 
				if (token_len > extra_len)
					iwe->u.data.pointer = NULL;		// Discard paylod
				// Check that the advertised token size is not going to produce buffer overflow to our caller... 
...
{
	struct wireless_scan *oldwscan = (struct wireless_scan *)NULL;
	// Now, let's decode the event
	switch (event->cmd) {
		case SIOCGIWAP:			// New cell description. Allocate new cell descriptor, zero it.
			oldwscan = wscan;
		...
		case SIOCGIWENCODE:
			...
			break;
		case IWEVQUAL:		// We don't get complete stats, only qual
			wscan->has_stats = 1;
		...
		case SIOCGIWRATE:
			...
			* a single bitrate, we only keep the largest one. */
			...
			break;
		case IWEVCUSTOM:	// How can we deal with those sanely ? Jean II
		default:
			break;
	}	// switch(event->cmd)
...
realloc:
	...
	if (wrq.u.data.length) {
		...
		struct wireless_scan *wscan = (struct wireless_scan *)NULL;
		int ret = -1;
#ifdef DEBUG
