...
static inline void iw_print_ie_wpa(unsigned char *iebuf, int buflen)
{
	...
	unsigned char *wpa_oui = (unsigned char *)NULL;
	int i = -1;
	uint16_t ver = 0;
	uint16_t cnt = 0;
...
static inline void print_scanning_token(stream, event, state, iw_range, has_range)
	...
	* from iwconfig... */
	...
	if (scanflags == IW_SCAN_HACK) // Skip waiting
		...
		if (iw_set_ext(skfd, ifname, SIOCSIWSCAN, &wrq) < 0) {
			...
			// If we don't have the permission to initiate the scan, we may still have permission to read left-over results. But, don't wait !!!
#if 0
...
realloc:
			...
			else		// We have the results, go to process them
				break;
		}
		// In here, check if event and event type if scan event, read results. All errors bad & no reset timeout
	}
...
static int 
print_freq_info(skfd, ifname, args, count)
int skfd;
char *ifname;
char *args[];					// Command line args
int count;						// Args count
...
static int print_bitrate_info(skfd, ifname, args, count)
int skfd;
char *ifname;
char *args[];			// Command line args
int count;				// Args count
...
static int 
print_keys_info(skfd, ifname, args, count)
int skfd;
char *ifname;
char *args[];			// Command line args
int count;				// Args count
{
	...
	if (iw_get_range_info(skfd, ifname, &range) < 0)
		...
		if (iw_get_ext(skfd, ifname, SIOCGIWENCODE, &wrq) >= 0) {
			// Note : if above fails, we have already printed an error message int the loop above
...
static int get_pm_value(int skfd, char *ifname, struct iwreq *pwrq, int flags, char *buffer, int buflen, int we_version_compiled)
{
...
static void print_pm_value_range(char *name, int mask, int iwr_flags, int iwr_min, int iwr_max, char *buffer, int buflen, int we_version_compiled)
{
	if (iwr_flags & mask) {
...
// Print Power Management info for each device
static int print_pm_info(skfd, ifname, args, count)
int skfd; 
char *ifname;
char *args[];			// Command line args
int count;				// Args count
{
	...
	if ((iw_get_range_info(skfd, ifname, &range) < 0) || (range.we_version_compiled < 10))
		...
		print_pm_value_range("period ", IW_POWER_PERIOD, range.pmp_flags, range.min_pmp, range.max_pmp, buffer, sizeof(buffer), range.we_version_compiled);
		// Display min/max timeout availables
		print_pm_value_range("timeout", IW_POWER_TIMEOUT, range.pmt_flags, range.min_pmt, range.max_pmt, buffer, sizeof(buffer), range.we_version_compiled);
		...
		if (iw_get_ext(skfd, ifname, SIOCGIWPOWER, &wrq) >= 0) {
			...
			if (wrq.u.power.disabled)
				...
				while (1) {			// Deal with min/max for the current value
					pm_mask = 0;
					...
					flags = get_pm_value(skfd, ifname, &wrq, pm_type, buffer, sizeof(buffer),
					range.we_version_compiled);
...
static int get_retry_value(int skfd, char *ifname, struct iwreq *pwrq, int flags, char *buffer, int buflen, int we_version_compiled)
{
...
// Print Power Management range for each type
static void print_retry_value_range(name, mask, iwr_flags, iwr_min, iwr_max, buffer, buflen, we_version_compiled)
char *name;
int mask;
int iwr_flags;
int iwr_min;
...
static int print_retry_info(skfd, ifname, args, count)
int skfd;
char *ifname;
char *args[];		// Command line args
int count;			// Args count
{
	...
	if ((iw_get_range_info(skfd, ifname, &range) < 0) || (range.we_version_compiled < 11))
		...
		print_retry_value_range("limit   ", IW_RETRY_LIMIT, range.retry_flags, range.min_retry, range.max_retry, buffer, sizeof(buffer), range.we_version_compiled);
		// Display min/max lifetime availables
		print_retry_value_range("lifetime", IW_RETRY_LIFETIME,  range.r_time_flags, range.min_r_time, range.max_r_time, buffer, sizeof(buffer), range.we_version_compiled);
			...
			if (wrq.u.retry.disabled)
				...
				while (1) {
					...
					if (retry_mask) {
						...
						get_retry_value(skfd, ifname, &wrq, retry_mask, buffer, sizeof(buffer), range.we_version_compiled);
					}
					// And if we have both a limit and a lifetime, ask the other one. 
					...
					// Ask for this other type of value
					flags = get_retry_value(skfd, ifname, &wrq, retry_type, buffer, sizeof(buffer), range.we_version_compiled);
...
// Note : now that we have scanning support, this is depracted and won't survive long. Actually, next version it's out !

// Display the list of ap addresses and the associated stats Exacly the same as the spy list, only with different IOCTL and messages
static int print_ap_info(skfd, ifname, args, count)
int skfd;
char *ifname;
char *args[];		// Command line args
int count;			// Args count
{
	...
	char temp[128];
	struct sockaddr *hwa = (struct sockaddr *)NULL;
	struct iw_quality *qual = (struct iw_quality *)NULL;
	iwrange range;
	int has_range = 0, has_qual = 0, n = -1, i = -1;
	// Avoid "Unused parameter" warning
	args = args; count = count;
	// Collect stats
...
static int 
print_event_capa_info(skfd, ifname, args, count)
int skfd;
char *ifname;
char *args[];		// Command line args
int count;			// Args count
...
static int 
print_auth_info(skfd, ifname, args, count)
int skfd;
char *ifname;
char *args[];		// Command line args
int count;			// Args count
...
static int print_wpakeys_info(skfd, ifname, args, count)
int skfd;
char *ifname;
char *args[];		// Command line args
int count;			// Args count
{
	...
	struct iw_encode_ext *extinfo = (struct iw_encode_ext *)NULL;
	unsigned int k = 0U;
	char buffer[128];
	// Avoid "Unused parameter" warning
	args = args; count = count;
 ...
 */
static int print_gen_ie_info(skfd, ifname, args, count)
int skfd;
char *ifname;
char *args[];		// Command line args
int count;			// Args count
...
/************************* COMMON UTILITIES *************************/
// This section was initially written by Michael Tokarev <mjt@tls.msk.ru> but heavily modified by me ;-)
...
{
	const iwlist_cmd *found = (iwlist_cmd *)NULL;
	int ambig = 0;
...
static void iw_usage(int status)
{
	...
	for (i = 0; iwlist_cmds[i].cmd != NULL; ++i)
		fprintf(f, "%s [interface] %s %s\n", (i? "             ": "Usage: iwlist"), iwlist_cmds[i].cmd, iwlist_cmds[i].argsname? iwlist_cmds[i].argsname: "");
