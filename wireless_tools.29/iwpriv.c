	...
	* don't have to bother with it later on... */
	...
	if ((priv[k].set_args & IW_PRIV_TYPE_MASK) && (priv[k].set_args & IW_PRIV_SIZE_MASK)) {
		switch (priv[k].set_args & IW_PRIV_TYPE_MASK) {
			case IW_PRIV_TYPE_BYTE:		// Number of args to fetch
				...
				break;
			case IW_PRIV_TYPE_INT:			// Number of args to fetch
				wrq.u.data.length = count;
				if (wrq.u.data.length > (priv[k].set_args & IW_PRIV_SIZE_MASK))
					wrq.u.data.length = priv[k].set_args & IW_PRIV_SIZE_MASK;
			...
			case IW_PRIV_TYPE_CHAR:
				...
				break;
			case IW_PRIV_TYPE_FLOAT:		// Number of args to fetch
				wrq.u.data.length = count;
				if (wrq.u.data.length > (priv[k].set_args & IW_PRIV_SIZE_MASK))
					wrq.u.data.length = priv[k].set_args & IW_PRIV_SIZE_MASK;
				...
				break;
			case IW_PRIV_TYPE_ADDR:		// Number of args to fetch
				wrq.u.data.length = count;
				if (wrq.u.data.length > (priv[k].set_args & IW_PRIV_SIZE_MASK))
					wrq.u.data.length = priv[k].set_args & IW_PRIV_SIZE_MASK;
	...
	// Those two tests are important. They define how the driver will have to handle the data
	...
	if ((priv[k].get_args & IW_PRIV_TYPE_MASK) && (priv[k].get_args & IW_PRIV_SIZE_MASK)) {
		...
		switch (priv[k].get_args & IW_PRIV_TYPE_MASK) {
			case IW_PRIV_TYPE_BYTE:	// Display args
				for (j = 0; j < n; j++)
					printf("%d  ", buffer[j]);
			...
			case IW_PRIV_TYPE_INT:	// Display args
				...
				printf("\n");
				break;
			case IW_PRIV_TYPE_CHAR:	// Display args
				buffer[n] = '\0';
				printf("%s\n", buffer);
				break;
...
char *ifname;		// Dev name
{
	...
	int ret = -1, number = -1;		// Max of private ioctl
...
static int print_priv_info(int skfd, char *ifname, char *args[], int count)
{
	...
	// Is there any ?
	if (n <= 0)			// Should I skip this message ?
		fprintf(stderr, "%-8.16s  no private ioctls.\n\n", ifname);
	else {
		...
		for (k = 0; k < n; k++) {
			if (priv[k].name[0] != '\0') {
				printf("          %-16.16s (%.4X) : set %3d %s & get %3d %s\n", \
					priv[k].name, priv[k].cmd, priv[k].set_args & IW_PRIV_SIZE_MASK, \
...
{
	struct iwreq wrq;
	int k = -1, number = -1, i = 0;		// Start with first arg
	iwprivargs *priv = (iwprivargs *)NULL;
	char ptype = 0x00;
	...
	if (count == 0) {
		...
		k = -1;
		while ((++k < number) && strcmp(priv[k].name, "gport_type") && strcmp(priv[k].name, "get_port"));
	...
	k = -1;
	while ((++k < number) && strcmp(priv[k].name, "sport_type") && strcmp(priv[k].name, "set_port"));
			...
			if ((!strncmp(argv[1], "-h", 2)) || (!strcmp(argv[1], "--help")))
				...
				if (!strcmp(argv[1], "-v") || !strcmp(argv[1], "--version"))
					...
					if (argc == 2)
						...
						// All
						if ((!strncmp(argv[2], "-a", 2)) || (!strcmp(argv[2], "--all")))
