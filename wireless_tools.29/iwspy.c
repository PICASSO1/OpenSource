...
static int print_spy_info(int skfd, char *ifname, char *args[], int count)
{
	...
	if ((n > 0) && (has_range) && (range.we_version_compiled > 11)) {
		...
		if (iw_get_stats(skfd, ifname, &stats, &range, has_range) >= 0) {
			...
			iw_print_stats(temp, sizeof(temp), &range.avg_qual, &range, has_range);
...
int count;		// Args count
{
	...
	if ((has_range) && (threshold.low.level)) {
		...
		printf("%-8.16s  Low threshold:%d dBm  High threshold:%d dBm\n\n", \
			ifname, threshold.low.level - 0x100, threshold.high.level - 0x100);
		}
		else {
			// Statistics are relative values (0 -> max)
			printf("%-8.16s  Low threshold:%d/%d  High threshold:%d/%d\n\n", \
			ifname, threshold.low.level, range.max_qual.level, threshold.high.level, range.max_qual.level);
		}
	}
	else {
		// We can't read the range, so we don't know...
		printf("%-8.16s  Low threshold:%d  High threshold:%d\n\n", \
			ifname, threshold.low.level, threshold.high.level);
	}
...
int count;		// Args count
{
	...
	if (!strcmp(args[0], "off")) {
		...
		if ((sscanf(args[0], "%i", &low_thr) != 1) || (sscanf(args[1], "%i", &high_thr) != 1)) {
...
char *envp[];
{
	int skfd = 0;			// generic raw socket desc. 
	int goterr = 0;
