...
static int 
sample_ioctl_set_mac(dev, info, data, mac_addr)
 ...
 */
static int 
sample_ioctl_set_addr(dev, info, mac_addr, extra)
struct net_device *dev;
struct iw_request_info *info;
struct sockaddr *mac_addr;
char *extra;
...
static int 
sample_ioctl_get_mac(dev, info, data, mac_addr)
struct net_device *dev;
struct iw_request_info *info;
struct iw_point *data;
struct sockaddr *mac_addr;
...
static int 
sample_ioctl_set_float(dev, info, freq, extra)
struct net_device *dev;
struct iw_request_info *info;
struct iw_freq *freq;
char *extra;
...
static const struct iw_priv_args sample_priv[] = {
	...
	// --- sub-ioctls handler ---
	{SIOCIWFIRSTPRIV + 0, IW_PRIV_TYPE_ADDR | IW_PRIV_SIZE_FIXED | 1, 0, "" },
	// --- sub-ioctls definitions ---
	{0, IW_PRIV_TYPE_ADDR | IW_PRIV_SIZE_FIXED | 1, 0, "macadd" },
	{1, IW_PRIV_TYPE_ADDR | IW_PRIV_SIZE_FIXED | 1, 0, "macdel" },
	{2, IW_PRIV_TYPE_ADDR | IW_PRIV_SIZE_FIXED | 1, 0, "mackick" },
	// *** Method 2 : binding one handler to multiple ioctls ***
	{SIOCIWFIRSTPRIV + 2, IW_PRIV_TYPE_ADDR | IW_PRIV_SIZE_FIXED | 1, 0, "addradd" },
	{SIOCIWFIRSTPRIV + 4, IW_PRIV_TYPE_ADDR | IW_PRIV_SIZE_FIXED | 1, 0, "addrdel" },
	// *** Extra fun ***
	{SIOCIWFIRSTPRIV + 1, 0, IW_PRIV_TYPE_ADDR | 16, "macget" },
	{SIOCIWFIRSTPRIV + 6, IW_PRIV_TYPE_FLOAT | IW_PRIV_SIZE_FIXED | 1, 0, "setfloat" },
};
...
static const iw_handler sample_private_handler[] = {
	...
	case SIOCIWFIRSTPRIV + 0:
		if (wrq->u.data.length > 1)
			...
			if (copy_from_user(&mac_addr, wrq->u.data.pointer, sizeof(struct sockaddr))) {
				ret = -EFAULT;
				break;
			}
			ret = sample_ioctl_set_mac(dev, NULL, &wrq->u.data, &mac_addr);
		}
		else { }
		break;
	...
	case SIOCIWFIRSTPRIV + 4:
		if (!capable(CAP_NET_ADMIN))
			...
			info.cmd = cmd;
			ret = sample_ioctl_set_addr(dev, &info, &wrq->u.ap_addr, NULL);
		}
		break;
	case SIOCIWFIRSTPRIV + 1:
		if (wrq->u.essid.pointer) {
			...
			ret = sample_ioctl_get_mac(dev, NULL, &wrq->u.data, mac_addr);
			if (copy_to_user(wrq->u.data.pointer, nickbuf, wrq->u.data.length * sizeof(struct sockaddr)))
				ret = -EFAULT;
		}
		break;
	case SIOCIWFIRSTPRIV + 6:
		if (!capable(CAP_NET_ADMIN))
			...
			ret = sample_ioctl_set_float(dev, NULL, &wrq->u.freq, NULL);
		break;
#endif		// WIRELESS_EXT < 15
